...
function renderMarkdown(content: string): string {
  return content
    ...
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    .replace(/^> (.+)$/gm, '<blockquote class="border-l-2 border-white/20 pl-4 text-gray-400 italic my-4">$1</blockquote>')
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-gray-200">
      <div className="max-w-2xl mx-auto px-6 py-16">
        ...
        {/* Title */}
        <h1 className="text-3xl md:text-4xl font-bold text-white leading-tight mb-6 tracking-tight">
          {note.title}
        </h1>

        {/* Tags */}
        <div className="flex gap-2 mb-12 pb-12 border-b border-white/5">
          {note.tags.map((tag) => (
