...
import type { PropertyType, PropertyStatus } from "@/lib/data";
...
export default function PropertiesPage() {
  ...
  const SidebarContent = () => (
    <div className="space-y-8">
      {/* Price range */}
      <div>
        <h3 className="text-xs font-bold tracking-[0.18em] uppercase text-text-dark mb-4">
          Price Range
        </h3>
        ...
        <div className="space-y-2">
          {propertyTypes.map((type) => (
            <label key={type} className="flex items-center gap-3 cursor-pointer group">
              ...
              />
              <span className="text-sm text-text-dark group-hover:text-primary transition-colors">
                {type}
              </span>
            </label>
          ))}
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap">
          {bedroomOptions.map((n) => (
            <button
              key={n}
              onClick={() => setFilters((f) => ({ ...f, bedrooms: f.bedrooms === n ? null : n }))}
              className={`w-10 h-10 text-sm font-semibold transition-all duration-200 ${
                filters.bedrooms === n
                  ? "bg-primary text-white"
                  : "border border-gray-200 text-text-muted hover:border-primary hover:text-primary"
              }`}
            >
              {n}+
            </button>
        ...
        <div className="flex gap-8 lg:gap-12">
          ...
          <aside className="hidden lg:block w-64 xl:w-72 flex-shrink-0">
            <div className="sticky top-28 bg-white border border-gray-100 p-6 shadow-sm">
              <h2 className="text-base font-bold text-text-dark mb-6 pb-4 border-b border-gray-100">
                Filter Properties
              </h2>
              <SidebarContent />
            </div>
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {filters.types.map((t) => (
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold bg-primary/10 text-primary px-3 py-1.5 hover:bg-primary/20 transition-colors"
                  >
                    {t} <X size={11} />
                  </button>
                ))}
                {filters.statuses.map((s) => (
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold bg-accent/10 text-accent px-3 py-1.5 hover:bg-accent/20 transition-colors"
                  >
                    {s} <X size={11} />
                  </button>
                ))}
                {filters.bedrooms !== null && (
                  <button
                    onClick={() => setFilters((f) => ({ ...f, bedrooms: null }))}
                    className="flex items-center gap-1.5 text-xs font-semibold bg-gray-100 text-text-muted px-3 py-1.5 hover:bg-gray-200 transition-colors"
                  >
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-24">
                ...
                <button onClick={clearFilters} className="text-primary font-semibold hover:text-accent transition-colors text-sm mt-2">
                  Clear all filters
                </button>
              </div>
            ) : (
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              ...
              <button onClick={() => setSidebarOpen(false)} className="p-1.5 text-text-muted hover:text-text-dark">
                <X size={20} />
              </button>
            </div>
